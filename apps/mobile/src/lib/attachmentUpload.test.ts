...
describe("prepareTurnAttachments", () => {
  beforeEach(() => {
    ...
    mocks.readAtom.mockReturnValue(
      Option.some({ httpBaseUrl: "https://environment.example/", queryParameters: [] }),
    );
  ...
  it("uses the current connection when an environment reconnects during URL creation", async () => {
    mocks.readAtom
      .mockReturnValueOnce(
        Option.some({ httpBaseUrl: "https://old-environment.example/", queryParameters: [] }),
      )
      .mockReturnValueOnce(
        Option.some({ httpBaseUrl: "https://new-environment.example/", queryParameters: [] }),
      );
