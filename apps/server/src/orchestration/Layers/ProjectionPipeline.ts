      ...
      switch (event.type) {
        ...
        case "thread.approval-response-requested":
          ...
          yield* pendingInteractionRepository.markResponding({
            ...
            action:
              ...
              event.payload.decision === "acceptAlways"
                ? "approve"
        ...
        case "thread.session-set":
          if (
            ...
            const openInteractions = yield* pendingInteractionRepository.listAllOpenByThreadId({
              threadId: event.payload.threadId,
            });
            const openUserInputRequestIds = new Set(
              openInteractions
                .filter((interaction) => interaction.kind === "user-input")
                .map((interaction) => interaction.requestId),
            );
            const messageModeRequestIds = new Set<ApprovalRequestId>();
            if (openUserInputRequestIds.size > 0) {
              ...
              for (const activity of userInputActivities) {
                ...
                const requestId = extractActivityRequestId(activity.payload);
                if (requestId === null || !openUserInputRequestIds.has(requestId)) {
                  continue;
                }
                const payload = decodeUserInputRequestedPayload(activity.payload);
                if (Option.isSome(payload) && payload.value.responseMode === "message") {
                  messageModeRequestIds.add(requestId);
                }
              }
            }
            yield* Effect.forEach(
              openInteractions.filter(
                (interaction) =>
                  interaction.kind === "approval" ||
                  (interaction.kind === "user-input" &&
                    !messageModeRequestIds.has(interaction.requestId)),
              ),
              (interaction) =>
                pendingInteractionRepository.markStale({
