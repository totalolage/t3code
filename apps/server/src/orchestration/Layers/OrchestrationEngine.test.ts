...
function makeOrchestrationLayer(databasePath?: string) {
  ...
  return Layer.mergeAll(
    ...
    PendingInteractionRepositoryLive,
  ).pipe(
...
describe("OrchestrationEngine", () => {
  ...
  it.each(["ready", "stopped"] as const)(
    "keeps a remote message-mode question answerable and idempotent after %s and projection restart",
    async (status) => {
      const directory = await NodeFSP.mkdtemp(
        NodePath.join(NodeOS.tmpdir(), "t3-remote-async-questions-"),
      );
      const databasePath = NodePath.join(directory, "state.sqlite");
      let system = await createOrchestrationSystem(databasePath);
      const threadId = ThreadId.make("remote-async-thread");
      const projectId = ProjectId.make("remote-async-project");
      const requestId = ApprovalRequestId.make("codex-remote-async:question-1");
      const authSessionId = AuthSessionId.make("remote-async-session");
      const idempotencyKey = RemoteInteractionIdempotencyKey.make("remote-async-answer-1");
      const dispatcher = {
        dispatch: (
          command: OrchestrationCommand,
          options?: { readonly origin?: OrchestrationClientOrigin },
        ) =>
          system.engine.dispatch(command, options).pipe(
            Effect.mapError(
              (cause) =>
                new OrchestrationDispatchCommandError({
                  message: "orchestration dispatch failed",
                  cause,
                }),
            ),
          ),
      ...
      try {
        await system.run(
          system.engine.dispatch({
            type: "project.create",
            commandId: CommandId.make("remote-async-project"),
            projectId,
            title: "Remote async questions",
            workspaceRoot: "/tmp/remote-async-questions",
            createdAt: now(),
          }),
        );
        await system.run(
          system.engine.dispatch({
            type: "thread.create",
            commandId: CommandId.make("remote-async-thread"),
            threadId,
            projectId,
            title: "Remote async questions",
            modelSelection: { instanceId: ProviderInstanceId.make("codex"), model: "gpt-5.4" },
        ...
        await system.run(
          system.engine.dispatch({
            type: "thread.activity.append",
            commandId: CommandId.make("remote-async-question"),
            threadId,
            createdAt: now(),
            activity: {
              id: EventId.make("remote-async-question"),
              kind: "user-input.requested",
              summary: "User input requested",
              tone: "info",
              turnId: null,
        ...
        await system.run(
          system.engine.dispatch({
            type: "thread.session.set",
            commandId: CommandId.make(`remote-async-session-${status}`),
            threadId,
        ...
        await system.dispose();
        system = await createOrchestrationSystem(databasePath);
        const pending = await system.run(listRemotePendingInteractions({ threadId }));
        expect(pending.interactions).toHaveLength(1);
        expect(pending.interactions[0]).toMatchObject({
          requestId,
          status: "pending",
        ...
        const response = {
          ...
          answers: [{ questionId: "0", values: ["pnpm"] }],
          dispatcher,
        };
        const invalidResponse = await system.run(
          Effect.flip(
            respondToRemotePendingInteraction({
              ...response,
              idempotencyKey: RemoteInteractionIdempotencyKey.make("remote-async-answer-invalid"),
              answers: [{ questionId: "0", values: ["pnpm", "npm"] }],
            }),
          ),
        );
        expect(invalidResponse).toMatchObject({
          _tag: "PendingInteractionInvalidResponseError",
          reason: "invalid_answers",
        });
        const first = await system.run(respondToRemotePendingInteraction(response));
        const retry = await system.run(respondToRemotePendingInteraction(response));
