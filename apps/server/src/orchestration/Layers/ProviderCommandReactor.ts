...
const make = Effect.gen(function* () {
  ...
  const registerInFlightTurnStart = (key: string) =>
    Effect.sync(() => inFlightTurnStartKeys.add(key)).pipe(
      Effect.andThen(TxRef.update(inFlightTurnStarts, (count) => count + 1).pipe(Effect.tx)),
    );

  const releaseInFlightTurnStart = (key: string) =>
  ...
  const processTurnStartRequested = Effect.fn("processTurnStartRequested")(function* (
    ...
    if (isCompactCommand) {
      ...
      yield* Effect.gen(function* () {
        ...
        Effect.matchCauseEffect({
          onFailure: (cause) =>
            Cause.hasInterruptsOnly(cause)
              ? Cache.invalidate(handledTurnStartKeys, key).pipe(Effect.andThen(Effect.interrupt))
    ...
    yield* providerService.sendTurn(sendTurnRequest.value).pipe(
      ...
      Effect.ensuring(releaseInFlightTurnStart(key)),
  ...
  const processCompactRequested = Effect.fn("processCompactRequested")(function* (
    ...
    const reason: ThreadCompactCompletionReason | undefined =
      ...
      (activeTurnId !== null && activeTurnId !== undefined)
        ? "active-thread"
        : yield* providerService.compactThread(event.payload.threadId).pipe(
            Effect.as(undefined),
            Effect.catch((error) =>
              Effect.succeed(
  ...
  const start: ProviderCommandReactorShape["start"] = Effect.fn("start")(function* () {
    ...
    const replayLifecycle = Effect.gen(function* () {
    ...
    if (activation === undefined) {
      ...
      yield* replayLifecycle;
    } else {
      yield* forkParked(clearInterrupted);
      yield* forkParked(replayLifecycle);
    }
  });
