...
it.layer(BaseTestLayer)("OrchestrationProjectionPipeline", (it) => {
  ...
  it.effect("clears stale pending user input from projected shell summaries", () =>
    Effect.gen(function* () {
      ...
      const eventStore = yield* OrchestrationEventStore;
      const sql = yield* SqlClient.SqlClient;
      const threadId = ThreadId.make("thread-stale-user-input-durable");
      const requestId = "user-input-request-stale-durable";
      const appendAndProject = (event: Parameters<typeof eventStore.append>[0]) =>
        eventStore
          .append(event)
          .pipe(Effect.flatMap((savedEvent) => projectionPipeline.projectEvent(savedEvent)));
      ...
      yield* appendAndProject({
        type: "project.created",
        eventId: EventId.make("evt-stale-user-input-durable-project"),
        aggregateKind: "project",
        aggregateId: ProjectId.make("project-stale-user-input-durable"),
        occurredAt: "2026-08-24T12:35:00.000Z",
        commandId: CommandId.make("cmd-stale-user-input-durable-project"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-stale-user-input-durable-project"),
        metadata: {},
        payload: {
          projectId: ProjectId.make("project-stale-user-input-durable"),
          title: "Project Stale User Input Durable",
          workspaceRoot: "/tmp/project-stale-user-input-durable",
          defaultModelSelection: null,
          scripts: [],
      ...
      yield* appendAndProject({
        type: "thread.created",
        eventId: EventId.make("evt-stale-user-input-durable-thread"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-08-24T12:35:01.000Z",
        commandId: CommandId.make("cmd-stale-user-input-durable-thread"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-stale-user-input-durable-thread"),
        metadata: {},
        payload: {
          threadId,
          projectId: ProjectId.make("project-stale-user-input-durable"),
          title: "Thread Stale User Input Durable",
          modelSelection: {
            instanceId: ProviderInstanceId.make("codex"),
            model: "gpt-5-codex",
          },
          ...
          branch: null,
          worktreePath: null,
          createdAt: "2026-08-24T12:35:01.000Z",
          updatedAt: "2026-08-24T12:35:01.000Z",
        },
      });

      yield* appendAndProject({
        type: "thread.activity-appended",
        eventId: EventId.make("evt-stale-user-input-durable-requested"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-08-24T12:35:02.000Z",
        commandId: CommandId.make("cmd-stale-user-input-durable-requested"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-stale-user-input-durable-requested"),
        metadata: {},
        payload: {
          threadId,
          activity: {
            id: EventId.make("activity-stale-user-input-durable-requested"),
            tone: "info",
            ...
            turnId: null,
            createdAt: "2026-08-24T12:35:02.000Z",
          },
        },
      });
      ...
      yield* appendAndProject({
        type: "thread.activity-appended",
        eventId: EventId.make("evt-stale-user-input-durable-failed"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: "2026-08-24T12:35:03.000Z",
        commandId: CommandId.make("cmd-stale-user-input-durable-failed"),
        causationEventId: null,
        correlationId: CorrelationId.make("cmd-stale-user-input-durable-failed"),
        metadata: {},
        payload: {
          threadId,
          activity: {
            id: EventId.make("activity-stale-user-input-durable-failed"),
            tone: "error",
            kind: "provider.user-input.respond.failed",
            summary: "Provider user input response failed",
            payload: {
              requestId,
              detail: `Unknown pending Codex user input request: ${requestId}`,
            },
            turnId: null,
            createdAt: "2026-08-24T12:35:03.000Z",
          },
        },
      });

      const threadRows = yield* sql<{ readonly pendingUserInputCount: number }>`
        SELECT pending_user_input_count AS "pendingUserInputCount"
        FROM projection_threads
        WHERE thread_id = ${threadId}
      `;
  ...
  it.effect(
    "stales native callbacks on session termination after 100 message-mode questions while keeping remote questions open",
    () =>
      Effect.gen(function* () {
        const projectionPipeline = yield* OrchestrationProjectionPipeline;
        const eventStore = yield* OrchestrationEventStore;
        const sql = yield* SqlClient.SqlClient;
        const threadId = ThreadId.make("thread-session-pending-interactions");
        const projectId = ProjectId.make("project-session-pending-interactions");
        const appendAndProject = (event: Parameters<typeof eventStore.append>[0]) =>
          eventStore
            .append(event)
            .pipe(Effect.flatMap((savedEvent) => projectionPipeline.projectEvent(savedEvent)));
        ...
        yield* appendAndProject({
          type: "project.created",
          eventId: EventId.make("evt-session-pending-project"),
          aggregateKind: "project",
          aggregateId: projectId,
          occurredAt: "2026-08-24T12:40:00.000Z",
          commandId: CommandId.make("cmd-session-pending-project"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-session-pending-project"),
          metadata: {},
          payload: {
            projectId,
            title: "Session pending interactions",
            workspaceRoot: "/tmp/project-session-pending-interactions",
            defaultModelSelection: null,
            scripts: [],
            createdAt: "2026-08-24T12:40:00.000Z",
            updatedAt: "2026-08-24T12:40:00.000Z",
          },
        });
        yield* appendAndProject({
          type: "thread.created",
          eventId: EventId.make("evt-session-pending-thread"),
          aggregateKind: "thread",
          aggregateId: threadId,
          occurredAt: "2026-08-24T12:40:01.000Z",
          commandId: CommandId.make("cmd-session-pending-thread"),
          causationEventId: null,
          correlationId: CorrelationId.make("cmd-session-pending-thread"),
          metadata: {},
          payload: {
            threadId,
            projectId,
            ...
            branch: null,
            worktreePath: null,
            createdAt: "2026-08-24T12:40:01.000Z",
            updatedAt: "2026-08-24T12:40:01.000Z",
          },
        });
        ...
        const appendRequest = (input: {
          ...
          appendAndProject({
            type: "thread.activity-appended",
            eventId: EventId.make(input.activityId),
            aggregateKind: "thread",
            aggregateId: threadId,
            ...
            payload: {
              ...
              activity: {
                ...
                payload: {
                  ...
                  ...(input.responseMode === "message" ? { responseMode: input.responseMode } : {}),
                  questions: [
                    {
                      id: "confirm",
                      header: "Confirm",
        ...
        for (let index = 0; index < 100; index += 1) {
          ...
          yield* appendRequest({
            requestId: `remote-message-mode-${suffix}`,
            activityId: `activity-session-message-mode-${suffix}`,
            commandId: `cmd-session-message-mode-${suffix}`,
            createdAt: `2026-08-24T12:40:02.${suffix}Z`,
            responseMode: "message",
          });
        }
        yield* appendRequest({
          requestId: "remote-native-callback",
          activityId: "activity-session-native-callback",
          commandId: "cmd-session-native-callback",
          createdAt: "2026-08-24T12:40:02.100Z",
        });

        // A full activity read would try to decode this unrelated malformed row.
        yield* sql`
        INSERT INTO projection_thread_activities (
          activity_id,
          thread_id,
          ...
          'activity-session-malformed-tool',
          ${threadId},
          NULL,
          'info',
          'tool.completed',
          ...
          '{not-json',
          NULL,
          '2026-08-24T12:40:04.000Z'
        )
      `;

        for (const [index, status] of (
          ["ready", "interrupted", "stopped", "error"] as const
        ).entries()) {
          const updatedAt = `2026-08-24T12:40:0${5 + index}.000Z`;
          yield* appendAndProject({
            type: "thread.session-set",
            eventId: EventId.make(`evt-session-pending-${status}`),
            aggregateKind: "thread",
            aggregateId: threadId,
            occurredAt: updatedAt,
            commandId: CommandId.make(`cmd-session-pending-${status}`),
            causationEventId: null,
            correlationId: CorrelationId.make(`cmd-session-pending-${status}`),
            metadata: {},
            payload: {
              threadId,
              session: {
                ...
                runtimeMode: "full-access",
                activeTurnId: null,
                lastError: status === "error" ? "session failed" : null,
                updatedAt,
              },
            },
          });
      ...
      `;
        const messageModeRows = interactionRows.filter(({ requestId }) =>
          requestId.startsWith("remote-message-mode-"),
        );
        assert.strictEqual(interactionRows.length, 101);
        assert.strictEqual(messageModeRows.length, 100);
        assert.isTrue(messageModeRows.every(({ status }) => status === "pending"));
        assert.deepEqual(
          interactionRows.find(({ requestId }) => requestId === "remote-native-callback"),
          { requestId: "remote-native-callback", status: "stale" },
        );
      }),
  );
