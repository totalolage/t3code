...
routing.layer("ProviderServiceLive native compaction", (it) => {
  it.effect("routes native compaction to Codex", () =>
    Effect.gen(function* () {
      const provider = yield* ProviderService.ProviderService;
      const threadId = asThreadId("thread-compact-codex");
      yield* provider.startSession(threadId, {
        provider: CODEX_DRIVER,
        providerInstanceId: codexInstanceId,
        threadId,
      ...
      assert.deepEqual(routing.codex.compactThread.mock.calls, [[threadId, undefined]]);
    }),
  );
});
