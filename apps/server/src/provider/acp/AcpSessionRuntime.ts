...
const defaultModelConfigRefreshTimeout = Duration.seconds(15);
...
> =>
  Effect.gen(function* () {
    ...
    const failPendingModelConfigRefresh = Effect.fn(
      "AcpSessionRuntime.failPendingModelConfigRefresh",
    )(function* (error: EffectAcpErrors.AcpError) {
      const pending = yield* Ref.getAndSet(pendingModelConfigRefreshRef, Option.none());
      if (Option.isSome(pending)) {
        yield* Deferred.fail(pending.value, error).pipe(Effect.ignore);
      }
    });
    ...
    const ensureConnected = Effect.gen(function* () {
      ...
      if (yield* Ref.get(stoppingRef)) {
        return yield* runtimeClosedError();
      }
    });
    ...
    const recordTermination = Effect.fn("AcpSessionRuntime.recordTermination")(function* (
      ...
      yield* failPendingModelConfigRefresh(error);
    ...
    const processSessionUpdate = (notification: EffectAcpSchema.SessionNotification) =>
      Ref.get(configOptionsSupportedRef).pipe(
        Effect.flatMap((configOptionsSupported) =>
          handleSessionUpdate({
            queue: eventQueue,
            modeStateRef,
            configOptionsRef,
            configOptionsSupported,
            toolCallsRef,
            assistantSegmentRef,
      ...
      notificationSemaphore.withPermit(
        Effect.gen(function* () {
          ...
          if (
            ...
            (yield* Ref.get(configOptionsSupportedRef))
          ) {
            pendingRefresh = yield* Ref.getAndSet(pendingModelConfigRefreshRef, Option.none());
          }
          yield* processSessionUpdate(notification);
          if (
            Option.isSome(pendingRefresh) &&
            notification.update.sessionUpdate === "config_option_update"
          ) {
    ...
    yield* Scope.addFinalizer(
      ...
      Effect.gen(function* () {
        ...
        yield* failPendingModelConfigRefresh(runtimeClosedError());
      ...
      setSessionModel: (modelId, meta) =>
        modelSelectionSemaphore.withPermit(
          Effect.gen(function* () {
            ...
            const configRefresh = yield* Deferred.make<
              ReadonlyArray<EffectAcpSchema.SessionConfigOption>,
              EffectAcpErrors.AcpError
            >();
            ...
            return yield* request.pipe(
              Effect.flatMap((response) =>
                Deferred.await(configRefresh).pipe(
                  Effect.timeoutOrElse({
                    duration: defaultModelConfigRefreshTimeout,
                    orElse: () =>
                      Effect.gen(function* () {
                        const error = new EffectAcpErrors.AcpTransportError({
                          operation: "call-rpc",
                          ...
                          detail:
                            "Timed out waiting for Hermes model configuration options to refresh.",
                          cause: undefined,
                        });
                        yield* retireRuntime(error);
...
const handleSessionUpdate = ({
  ...
  readonly configOptionsSupported: boolean;
  ...
  Effect.gen(function* () {
    if (configOptionsSupported && params.update.sessionUpdate === "config_option_update") {
