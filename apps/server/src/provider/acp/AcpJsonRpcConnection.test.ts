...
describe("AcpSessionRuntime", () => {
  ...
  it.effect("waits for each model's gated config catalog before allowing the next switch", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const firstSwitch = yield* runtime.setSessionModel("grok-4.6").pipe(Effect.forkChild);
        ...
        yield* runtime.notify(releaseModelConfigUpdateMethod, { modelId: "grok-4.6" });
        ...
        Effect.provide(
          AcpSessionRuntime.layer({
            ...
            requestLogger: (event) => {
              ...
              if (event.payload.modelId === "grok-4.6" && event.status === "succeeded") {
  ...
  it.effect("fails a pending model refresh when ACP terminates after set_model replies", () =>
    Effect.gen(function* () {
      const modelResponse = yield* Deferred.make<void>();
      yield* Effect.gen(function* () {
        const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
        ...
        yield* runtime.notify("_test/exit", {});
        const error = yield* Fiber.join(modelSwitch).pipe(Effect.flip);
        expect(error).toMatchObject({ _tag: "AcpProcessExitedError" });
      }).pipe(
        Effect.provide(
          AcpSessionRuntime.layer({
            authMethodId: "test",
  ...
  it.effect("fails a pending model refresh when the runtime scope closes", () =>
    Effect.gen(function* () {
      const scope = yield* Effect.acquireRelease(Scope.make(), (scope) =>
        Scope.close(scope, Exit.void),
      );
  ...
  it.effect("retires after a model refresh timeout and rejects later selections", () =>
    Effect.gen(function* () {
      const modelResponse = yield* Deferred.make<void>();
      const lateModelUpdate = yield* Deferred.make<void>();
      yield* Effect.gen(function* () {
        const runtime = yield* AcpSessionRuntime.AcpSessionRuntime;
        yield* runtime.handleUnknownExtNotification((method, params) =>
          acknowledgedModelId(method, params) === "grok-4.6"
            ? Deferred.succeed(lateModelUpdate, undefined).pipe(Effect.ignore)
            : Effect.void,
        );
        yield* runtime.start();
        ...
        const firstError = yield* Fiber.join(firstSwitch).pipe(Effect.flip);
        expect(firstError).toMatchObject({
          _tag: "AcpTransportError",
          method: "session/set_model",
          detail: "Timed out waiting for Hermes model configuration options to refresh.",
        });

        const secondError = yield* runtime.setSessionModel("grok-mock-alt").pipe(Effect.flip);
        expect(secondError).toBe(firstError);

        const lateReleaseError = yield* runtime
          .notify(releaseModelConfigUpdateMethod, { modelId: "grok-4.6" })
          .pipe(Effect.flip);
        expect(lateReleaseError).toBe(firstError);
        expect(Option.isNone(yield* Deferred.poll(lateModelUpdate))).toBe(true);
        expect(yield* runtime.getConfigOptions).toEqual([]);
      }).pipe(
        Effect.provide(
          AcpSessionRuntime.layer({
