...
import * as OrchestrationCommandDispatcher from "./orchestration/Services/OrchestrationCommandDispatcher.ts";
...
const makeWsRpcLayer = (
  ...
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      // Every thread.create dispatched through the hardened bootstrap
      // dispatcher uses its created event as the deletion-cleanup fence: the
      // reactor stops sessions and closes terminals by the reused thread id,
      // so the new incarnation must not own resources until cleanup drained.
      const deletionFencedEngine: typeof orchestrationEngine = {
        ...orchestrationEngine,
      ...
      const orchestrationCommandDispatcher = yield* OrchestrationCommandDispatcher.make.pipe(
        Effect.provide(
          Layer.succeed(OrchestrationEngine.OrchestrationEngineService, deletionFencedEngine),
        ),
      );
      ...
      const dispatchNormalizedCommand = (command: OrchestrationCommand) =>
        orchestrationCommandDispatcher.dispatch(
          command,
      ...
      return WsRpcGroup.of({
        ...
        [WS_METHODS.serverCancelServiceUpdate]: (_input) =>
          observeRpcEffect(
            WS_METHODS.serverCancelServiceUpdate,
            Effect.gen(function* () {
              const cancelled = yield* serviceUpdateCoordinator.cancelDrain;
              return { cancelled };
            }),
            { "rpc.aggregate": "server" },
          ),
