...
export async function resolveFileAttachmentUrl(input: {
  ...
  const url = resolveAssetUrl(input.httpBaseUrl, result.value.relativeUrl, input.queryParameters);
...
export function hasServerAcknowledgedLocalDispatch(input: {
  ...
  if (input.isLatestUserMessageQueuedForServiceUpdate && latestUserMessageChanged) {
    return true;
  }
  if (
