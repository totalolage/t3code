...
function ProviderEnvironmentSection(props: {
  ...
  const publishRows = (nextRows: ReadonlyArray<EnvironmentDraftRow>) => {
    ...
    // Publish managed (hidden) variables back untouched, after the user's
    // rows; record the full array so the external-change effect recognizes
    // our own publish and does not reset the draft rows.
    const hidden = props.environment.filter((variable) => props.hiddenNames?.has(variable.name));
    const nextEnvironment = [...hidden, ...published];
    lastPublishedEnvironmentRef.current = nextEnvironment;
    props.onChange(nextEnvironment);
  };
