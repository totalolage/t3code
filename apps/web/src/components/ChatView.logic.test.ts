...
describe("hasServerAcknowledgedLocalDispatch", () => {
  ...
  it("does not acknowledge an unchanged message that is queued for a service update", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
    );
  ...
  it("acknowledges a newly projected message that is queued for a service update", () => {
    const localDispatch = createLocalDispatchSnapshot(
      makeThread({ latestTurn: completedTurn, session: readySession }),
    );
    ...
    expect(
      hasServerAcknowledgedLocalDispatch({
        ...
        phase: "ready",
        latestTurn: completedTurn,
        latestUserMessageId: MessageId.make("message-queued-follow-up"),
        session: readySession,
        hasPendingApproval: false,
        hasPendingUserInput: false,
        isLatestUserMessageQueuedForServiceUpdate: true,
