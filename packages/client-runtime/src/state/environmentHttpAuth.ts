...
export const executeAuthenticatedEnvironmentHttpRequest = Effect.fn(
  ...
  const queryParameters = input.prepared.queryParameters;
  return yield* Effect.gen(function* () {
    ...
    for (;;) {
      ...
      const client = yield* makeEnvironmentHttpApiClient(httpBaseUrl, queryParameters);
      ...
      orElse: () =>
        Effect.fail(
          new RemoteEnvironmentAuthTimeoutError(
            redactRequestUrl(input.url(httpBaseUrl)),
            input.timeoutMs,
          ),
        ),
    }),
