...
const ENVIRONMENT_QUERY = "?proxy=fork+route";
const REDACTED_ENVIRONMENT_QUERY = "?proxy=%5BREDACTED%5D";
const DIFF = {
...
describe("authenticated environment HTTP requests", () => {
      ...
      expect(harness.proofs).toEqual([
        {
          ...
          url: `${CURRENT_ORIGIN}${loader.path}${ENVIRONMENT_QUERY}`,
  ...
  it.effect("retries a rejected diff once with a new token, endpoint, and proof", () =>
    Effect.gen(function* () {
      ...
      expect(harness.calls.map((call) => call.url)).toEqual([
        `${CURRENT_ORIGIN}/api/pull-requests/diff${ENVIRONMENT_QUERY}`,
        `${RENEWED_ORIGIN}/api/pull-requests/diff${ENVIRONMENT_QUERY}`,
      ]);
      expect(
      ...
      expect(harness.proofs[1]).toEqual({
        method: "POST",
        url: `${RENEWED_ORIGIN}/api/pull-requests/diff${ENVIRONMENT_QUERY}`,
  ...
  it.effect("keeps the caller's timeout while waiting for renewal", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        ...
        requestUrl: `${PREPARED.httpBaseUrl}/api/auth/session${REDACTED_ENVIRONMENT_QUERY}`,
        timeoutMs: 100,
      });
      expect(error.message).toContain(REDACTED_ENVIRONMENT_QUERY);
      expect(error.message).not.toContain("fork+route");
      expect(Cause.pretty(Cause.fail(error))).not.toContain("fork+route");
  ...
  it.effect("reports the current endpoint when the total timeout expires during HTTP", () =>
    Effect.gen(function* () {
      ...
      expect(error).toMatchObject({
        _tag: "RemoteEnvironmentAuthTimeoutError",
        requestUrl: `${CURRENT_ORIGIN}/api/auth/session${REDACTED_ENVIRONMENT_QUERY}`,
        timeoutMs: 100,
      });
      expect(error.message).toContain(REDACTED_ENVIRONMENT_QUERY);
      expect(error.message).not.toContain("fork+route");
      expect(Cause.pretty(Cause.fail(error))).not.toContain("fork+route");
      expect(harness.calls.map((call) => call.url)).toEqual([
        `${CURRENT_ORIGIN}/api/auth/session${ENVIRONMENT_QUERY}`,
      ]);
