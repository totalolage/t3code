...
open the **T3 Code** tab. **Install and start** reads the release tag at the repository's current
checkout, downloads that tag's checksum-verified native artifact, activates the s6 service, and
verifies the live process and HTTP endpoint. The current release workflow publishes the companion
binary for Linux x64; ARM64 Hermes hosts are rejected until a Linux ARM64 standalone artifact is
available.
...
Hermes owns the plugin repository lifecycle. Use Hermes's plugin update action or
`hermes plugins update t3code` to pull the repository first. T3's dashboard **Update service** action
never pulls, checks out, resets, advances, or rolls back Git source. It treats the single release tag
at the current checkout as the desired service release.

The dashboard compares only that desired repository tag with the installed service release tag. If
they match, **Check for update** returns a successful no-op without a network request or service
restart. If they differ, it downloads the native artifact for the repository tag, verifies the
adjacent SHA-256 checksum and reported version, replaces and restarts the supervised service, and
reports success only after the new process owns the configured listener and passes supervisor,
...
`$HERMES_HOME/t3code/service-state.json`. A successful **Install and start** or **Update service**
records `installed` together with the installed release tag, product version, installed binary
...
against the digest recorded at install time and restores that same installed release. It is never an
implicit upgrade and does not compare the current repository checkout with the installed service
release. A missing, non-executable, changed, or architecture-incompatible binary leaves the Hermes
dashboard running; the plugin status reports
...
an installed release and durable intent. From then on, boot recovery is automatic and **Remove
