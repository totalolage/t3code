...
Pending interaction HTTP endpoints use the same scope split. Listing requires
`orchestration:read`; `answer`, `approve`, `decline`, and `cancel` require
`orchestration:operate`. Authentication and scope checks run before keyed lookup,
so callers without operate authority cannot use response IDs as an existence
oracle. Authorized missing or stale keys receive the same generic not-found
shape, and invalid response documents receive a generic invalid-interaction
error.
