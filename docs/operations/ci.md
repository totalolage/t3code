...
- `.github/workflows/f8y-release.yml` publishes an unsigned Apple Silicon DMG and one persistently signed Android APK as an `f8y` prerelease after every successful build of `main`.
...
- See [f8y Continuous Releases](./f8y-release.md) for the fork pipeline, Android keystore, and Obtainium setup.
