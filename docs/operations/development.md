...
Build the fork's Android APK and standalone CLI artifacts with:

```sh
vp run dist:mobile:apk:f8y
vp run --filter t3 build:binary:darwin-arm64
vp run --filter t3 build:binary:linux-x64
```

See the [f8y continuous releases](./f8y-release.md) runbook for packaging and release checks.
